 ...
 */
function link(output, folder, done) {
  _filesOfType(folder, 'out', function(error, files) {
    _link(output, files, done);
  });
}

function _link(output, files, done) {
  var paths = _.reduce(files, function(built, path) {
    return built + ' ' + path;
  }, '');

  exec(format(environment.native() + 'avr/bin/avr-gcc ', linking, ' -o ' + output + ' ' + paths + ' bin/main.a -L -lm'), function(error, stdout, stderr) {
  ...
  execute: function(options) {
    async.series([
      function(cb) {
        ...
        recursiveCompile('src/', ['common/pins/standard/', 'arduino/'], function(output) {
          console.log(output)
        }, cb);
      },
      function(cb) {
        recursiveCompile(environment.install() + 'headers/arduino/', ['common/pins/standard/', 'arduino/'], function(output) {
          console.log(output);
        }, cb);
      },
      function(cb) {
      ...
      function(cb) {
        link('bin/main.elf', 'bin/arduino/', cb);
      },
      function(cb) {
