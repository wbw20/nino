    ...
    async.series([
      function(cb) {
        exec(environment.native() + 'avr/bin/avr-gcc -Os -DF_CPU=16000000UL -mmcu=atmega328p -o bin/main.out src/main.c', function(error, stdout, stderr) {
          cb(error, 'avr gcc build 1');
        });
      },
      function(cb) {
        exec(environment.native() + 'avr/bin/avr-gcc -mmcu=atmega328p bin/main.out -o main', function(error, stdout, stderr) {
          cb(error, 'avr gcc build 2');
        });
      },
