 ...
 */
function recursiveCompile(folder, cb) {
  _files(folder, function(error, files) {
    cb(async.eachLimit(files, 30, function(file) {
      compile(file, '~/nino_testing/blah.out', cArgs);
    }));
  });
}
...
module.exports = {
  execute: function(options) {
    ...
    recursiveCompile(environment.headers(), function(data) {
      console.log(data);
    });
  }
};
