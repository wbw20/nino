...
var sArgs = {
    ...
    common = ' -MMD -DUSB_VID=null -DUSB_PID=null -DARDUINO=105 -mmcu=atmega328p -DF_CPU=16000000L ';
...
function compile(file, output, libraries, cb) {
  switch (file.split('.').pop()) {
    case 'S':
      _compile(file, output, libraries, sArgs, cb); break;
    case 'c':
      _compile(file, output, libraries, cArgs, cb); break;
    case 'cpp':
      _compile(file, output, libraries, cppArgs, cb); break;
    case 'h':
      cb('Skipping ' + file);
      break;
    default:
...
function _compile(file, output, libraries, args, cb) {
  linkingOptions(libraries, function(libraryPaths) {
    exec(format(environment.native() + 'avr/bin/avr-gcc ', args, common + libraryPaths + ' ' + file + ' -o ' + output + file.split('/').pop().split('.')[0] + '.out'), function(error, stdout, stderr) {
...
function link(output, file, archive, library, done) {
  exec(format(environment.native() + 'avr/bin/avr-gcc ', linking, ' -o ' + output + ' ' + file + ' ' + archive + ' -L ' + library + ' -lm'), function(error, stdout, stderr) {
  ...
  execute: function(options) {
    async.series([
      function(cb) {
        ...
        recursiveCompile('src/', 'bin/', ['common/pins/standard/', 'arduino/'], function(output) {
          console.log(output)
        }, cb);
      },
      function(cb) {
      ...
      function(cb) {
        link('bin/main.elf', 'bin/driver.out', 'bin/main.a', 'bin/arduino/', cb);
      },
      function(cb) {
        objcopy('bin/main.elf', 'bin/main.eep', transform, cb);
      },
      function(cb) {
        objcopy('bin/main.elf', 'bin/main.hex', copy, cb);
      }
