...
/*
 * Link compiled files using avr-ar
 */
function link(folder, output, cb) {
  _files(folder, function(error, files) {
    async.queue(function(file, callback) {
      _link(file, output, callback);
    }, 3).push(files, cb);
  });
}

function _link(file, output, cb) {
  exec(environment.native() + 'avr/bin/avr-ar rcs ' + output, function(error, stdout, stderr) {
    if (error) { cb(error); return; }
    cb('Finished linking ' + file);
  });
}
...
module.exports = {
   ...
   *   2.  Compile core
   *   3.  Collect core into a library file
   *   4.  Link it into an .elf file
   */
  execute: function(options) {
    async.series([
      ...
      // },
      function(cb) {
        recursiveCompile(environment.install() + 'headers/arduino/', ['arduino/pins/standard/', 'arduino/'], cb);
      },
      // function(cb) {
      //   // link()
      // },
